...
test('injected wallet covers reject, reconnect, chain recovery, auth, money, and expiry states', async ({ page }) => {
  ...
  const switchNetwork = primaryNavigation.getByRole('button', { name: 'Switch wallet to Robinhood Chain' })
  await expect(switchNetwork).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Switch network to continue' })).toBeVisible()
  ...
  await page.getByLabel('Amount (USDG)').fill('101')
  await page.getByRole('button', { name: 'Deposit onchain' }).click()
  await expect(page.getByText(/capped at \$100\.00 synthetic USDG/i)).toBeVisible()

  await page.getByLabel('Amount (USDG)').fill('2')
  await page.getByRole('button', { name: 'Deposit onchain' }).click()
  await expect(page.getByText('$14.50', { exact: true })).toBeVisible()
  await expect(page.getByText('Deposit confirmed and indexed.')).toBeVisible()
  ...
  await expect(page.getByText(/tracking this exact call/i)).toBeVisible()
  await expect(page.getByRole('link', { name: /Inspect settlement/i })).toHaveAttribute(
    'href',
    /explorer\.testnet\.chain\.robinhood\.com\/tx\/0x/
  )
  await expect(page.locator('pre.output-block')).toContainText('execution verified', { timeout: 10_000 })
  expect(state.runAttempts).toBe(1)
  expect(new Set(state.runIdempotencyKeys).size).toBe(1)

  await page.goto('/builder')
  await expect(page.getByText('$5.02', { exact: true })).toBeVisible()
  await page.getByLabel('Amount (USDG)').fill('6')
  await page.getByRole('button', { name: 'Claim to wallet' }).click()
  await expect(page.getByText('Claim amount cannot exceed your available earnings.')).toBeVisible()

  await page.getByLabel('Amount (USDG)').fill('0.50')
  await page.getByRole('button', { name: 'Claim to wallet' }).click()
  await expect(page.getByText('$4.52', { exact: true })).toBeVisible()
  await expect(page.getByText('Claim confirmed and indexed.')).toBeVisible()

  await page.evaluate(async (nextAccount) => {
    const ethereum = (window as Window & {
      ethereum?: { request(args: { method: string; params?: unknown[] }): Promise<unknown> }
    }).ethereum
    await ethereum?.request({ method: 'velostra_setAccount', params: [nextAccount] })
  }, SECOND_TEST_WALLET)
  await expect(page.getByRole('heading', { name: 'Verify your wallet' })).toBeVisible()
  await expect(page.getByText('$4.52', { exact: true })).toHaveCount(0)
...
test('public testnet reports deep-readiness drift without claiming the runtime is live', async ({ page }) => {
  const state = createProductState()
  state.ready = false
  await installProductApi(page, state)

  await page.goto('/testnet')
  await expect(page.getByText('RUNTIME DEGRADED', { exact: true })).toBeVisible()
  await expect(page.getByText('TESTNET LIVE', { exact: true })).toHaveCount(0)
  await expect(page.getByText('Unavailable', { exact: true })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Recheck runtime' })).toBeVisible()
})
